								...
								<form class="admin-form" action="{{ route('back.slider.update', $slider->id) }}"
									...
									@if ($slider->home_page != 'theme4')
										{{-- <div class="form-group">
											<label id="change_label" for="name">{{ $slider->home_page == 'theme3' ||
												$slider->home_page == 'theme4' ? __('Feature Image') : __('Logo') }}</label>
											<br>
											<img class="admin-img"
												src="{{ $slider->logo ? url('assets/img/'.$slider->logo) : url('assets/img/placeholder.png') }}"
												alt="No Image Found">
											<br>
											<span id="change_message" class="mt-1">{{ $slider->home_page == 'theme3' ||
												$slider->home_page == 'theme4' ? __('Image Size Should Be 435 x 530') :
												__('Image Size Should Be 130 x 40')}}</span>
										</div> --}}
										...
										<div class="form-group">
											...
											<textarea name="details" id="details" class="form-control" rows="5"
												placeholder="{{ __('Enter Details') }}">{{ $slider->details }}</textarea>
										</div>
										...
										<div class="form-group">
											...
											<div class="d-flex align-items-center" style="gap:8px;">
												<input type="color"
													name="text_color"
													id="text_color"
													value="{{ $slider->text_color ?? '#ffffff' }}"
													style="height:38px;width:70px;padding:2px 4px;cursor:pointer;border:1px solid #ddd;border-radius:4px;">
												<span id="text_color_preview"
													style="padding:4px 14px;border-radius:4px;background:#2d3748;font-weight:600;font-size:13px;color:{{ $slider->text_color ?? '#ffffff' }};">
													{{ __('Preview') }}
												</span>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<br>
											<img class="admin-img"
												src="{{ $slider->photo ? url('assets/img/' . $slider->photo) : url('assets/img/placeholder.png') }}"
												alt="No Image Found">
											<br>
									...
									@else
										...
										<input name="details" type="hidden" id="details" value="theme4" class="form-control"
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
		...
		const preview = document.getElementById('text_color_preview');
