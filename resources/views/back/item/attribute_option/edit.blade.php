                            ...
                            <form class="admin-form"
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="attribute_id" class="form-control" id="attribute_id">
                                        ...
                                        <option value="{{ $attribute->id }}" {{ $attribute->id == $option->attribute_id
                                ...
                                <div class="row pb-4" id="image-gallery" style="margin-left:0">
                                    @php
                                    $variationImg = json_decode($option->variation_images, true);
                                    $galleryPhotos = $item->galleries->pluck('photo')->toArray();
                                    $mainSelected = in_array($item->photo, $variationImg ?? []);
                                    @endphp
                                    {{-- Main Product Image --}}
                                    {{-- @if($item->photo)
                                    <div class="col-lg-3 col-md-12 mb-4 mb-lg-0">
                                        <img src="{{ url('assets/img/' . $item->photo) }}"
                                            ...
                                            class="w-100 shadow-1-strong rounded mb-4 selectable-img {{ $mainSelected ? 'selected' : '' }}"
                                            alt="product" />
                                    </div>
                                    @endif --}}
                                    ...
                                    @php
                                    $isSelected = in_array($gallery->photo, $variationImg ?? []);
                                    @endphp
                                    <div class="col-lg-3 col-md-12 mb-4 mb-lg-0">
                                        <img src="{{ $gallery->photo ? url('assets/img/' . $gallery->photo) : url('assets/img/placeholder.png') }}"
                                            data-image="{{ $gallery->photo }}"
                                            class="w-100 shadow-1-strong rounded mb-4 selectable-img {{ $isSelected ? 'selected' : '' }}"
                                            alt="gallery" />
                                        @if($isSelected)
                                        <small class="text-primary d-block text-center">Selected</small>
                                        @endif
                                    </div>
                                    @empty
                                    ...
                                    {{-- Show variation images NOT in gallery --}}
                                    @php
                                    $remainingVariationImages = array_diff($variationImg ?? [], $galleryPhotos);
                                    @endphp

                                    @foreach($remainingVariationImages as $image)
                                    <div class="col-lg-3 col-md-12 mb-4 mb-lg-0">
                                        <img src="{{ url('assets/img/' . $image) }}" data-image="{{ $image }}"
                                            class="w-100 shadow-1-strong rounded mb-4 selectable-img selected"
                                            alt="variation" />
                                    </div>
                                    @endforeach
