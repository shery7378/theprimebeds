...
<tr id="product-bulk-delete">
  <td><input type="checkbox" class="bulk-item" value="{{$data->id}}"></td>
    <td>
        <img src="{{ $data->thumbnail ? url('assets/img/'.$data->thumbnail) : url('assets/img/placeholder.png') }}" alt="Image Not Found">
    </td>
    <td>
        {{ $data->name }}
    </td>
    ...
    <td>
        {{ $data->purchase_price ? PriceHelper::adminCurrencyPrice($data->purchase_price) : '—' }}
    </td>
    <td>
        <div class="dropdown">
