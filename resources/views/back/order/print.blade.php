...
<head>
  ...
  <link rel="icon"  type="image/x-icon" href="{{ url('assets/img/'.$setting->favicon) }}"/>
                                ...
                                <div class="gd-responsive-table">
                                    ...
                                    <tr>
                                        <td class="px-0">
                                            {{$item['name']}}
                                            @if(!empty($item['is_customized']))
                                                <span style="display:inline-block;background:#e8f4fd;color:#0c63e4;font-size:10px;padding:1px 6px;border-radius:8px;margin-left:4px;font-weight:600;">Customized</span>
                                            @endif
                                        </td>
                                        <td class="px-0">
                                            @if(!empty($item['is_customized']))
                                                ...
                                                @foreach ($item['customizations'] ?? [] as $custId => $customization)
                                                    <span class="entry-meta d-block">
                                                        <b>{{ $customization['customizer_name'] ?? '' }}</b>:
                                                        {{ $customization['option_name'] ?? '' }}
                                                        @if(isset($customization['option_price']) && $customization['option_price'] != 0)
                                                            @if ($setting->currency_direction == 1)
                                                                ({{ $customization['option_price'] > 0 ? '+' : '' }}{{$order->currency_sign}}{{round(abs($customization['option_price'])*$order->currency_value,2)}})
                                                            @else
                                                                ({{ $customization['option_price'] > 0 ? '+' : '' }}{{round(abs($customization['option_price'])*$order->currency_value,2)}}{{$order->currency_sign}})
                                                            @endif
                                                        @endif
                                                    </span>
                                                @endforeach
                                        ...
                                        <td class="px-0 text-right">
                                            @if(!empty($item['is_customized']))
                                                @php $linePrice = ($item['price'] ?? 0) * ($item['quantity'] ?? 1); @endphp
                                                @if ($setting->currency_direction == 1)
                                                    {{$order->currency_sign}}{{round($linePrice*$order->currency_value,2)}}
                                                @else
                                                    {{round($linePrice*$order->currency_value,2)}}{{$order->currency_sign}}
                                                @endif
                                            @else
                                                @if ($setting->currency_direction == 1)
