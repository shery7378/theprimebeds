...
class InvictaLuxeKidsBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Invicta Luxe Kids Bed%')->first();
        if (!$item) {
            echo "Invicta Luxe Kids Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('invicta_kids.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['50', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Invicta Luxe Kids Bed fully updated with attributes and images!\n";
    }
}
