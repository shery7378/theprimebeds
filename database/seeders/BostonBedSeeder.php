...
class BostonBedSeeder extends Seeder
{
    public function run(): void
    {
        // Must exclude Boston-X Bed if we just look for Boston Bed
        $item = DB::table('items')->where('name', 'like', '%Boston Bed%')->where('name', 'not like', '%Boston-X%')->first();
        if (!$item) {
            echo "Boston Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('boston-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                // Skip size icons (SINGLE, DOUBLE, KING, SUPER-KING, EMPEROR, SMALL-DOUBLE)
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston-X|Hyatt|Manorhouse|Vermont)-/i', $imgUrl) && stripos($imgUrl, 'Boston-Bed') === false && stripos($imgUrl, 'Boston_Bed') === false && !preg_match('/Boston-Bed-Compress/i', $imgUrl)) continue;

                // Only include if it's a real Boston Bed image (and not Boston-X)
                if (stripos($imgUrl, 'Boston') !== false && stripos($imgUrl, 'Boston-X') === false) {
        ...
        echo "Boston Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
