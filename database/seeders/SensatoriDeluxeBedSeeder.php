...
class SensatoriDeluxeBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Sensatori Deluxe Bed%')->first();
        if (!$item) {
            echo "Sensatori Deluxe Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('sensatori_deluxe.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['60', '72', '84', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Sensatori Deluxe Bed fully updated with attributes and images!\n";
    }
}
