...
class PerriBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Perri Bed%')->first();
        if (!$item) {
            echo "Perri Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('perri-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson)-/i', $imgUrl) && stripos($imgUrl, 'Perri') === false) continue;

                // Only include if it's a real Perri Bed image
                if (stripos($imgUrl, 'Perri') !== false) {
        ...
        echo "Perri Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
