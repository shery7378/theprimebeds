...
class BellagioXBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Bellagio-X Bed%High Sides%')->first();
        if (!$item) {
            // Also try without the hyphen or exact name
            $item = DB::table('items')->where('name', 'like', '%Bellagio-X Bed%')->first();
        }

        if (!$item) {
            echo "Bellagio-X Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('bellagio_x.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['60', '72', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Bellagio-X Bed fully updated with attributes and images!\n";
    }
}
