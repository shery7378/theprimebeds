...
class SensatoriDivanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Sensatori Divan Bed')->first();
        if (!$item) {
            echo "Sensatori Divan Bed not found. (Exact match failed, trying LIKE)\n";
            $item = DB::table('items')->where('name', 'like', '%Sensatori Divan Bed%')
                      ->where('name', 'not like', '%Hybrid%')
                      ->where('name', 'not like', '%Ottoman%')
                      ->first();
        }
        
        if (!$item) {
            echo "Sensatori Divan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('sensatori_divan.json'));
        ...
        echo "Sensatori Divan Bed fully updated with attributes and images!\n";
    }
}
