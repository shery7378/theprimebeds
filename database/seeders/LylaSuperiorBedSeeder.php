...
class LylaSuperiorBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Lyla Superior Bed')->first();
        if (!$item) {
            echo "Lyla Superior Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('lyla-superior-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['54', '60', '72', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'HEADBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Lyla Superior Bed fully updated with attributes and images!\n";
    }
}
