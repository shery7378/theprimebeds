...
class AmbassadorBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Ambassador Bed%')->first();
        if (!$item) {
            echo "Ambassador Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ambassador-bed.json'));
        ...
        // Gallery – only actual Ambassador Bed product photos
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori)-/i', $imgUrl) && stripos($imgUrl, 'Ambassador') === false) continue;

                // Only include if it's a real Ambassador Bed image
                if (stripos($imgUrl, 'Ambassador') !== false) {
        ...
        echo "Ambassador Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
