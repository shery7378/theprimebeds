...
class SonoGlassBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Sono Glass Bed%')->first();
        if (!$item) {
            echo "Sono Glass Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('sono-glass-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston|Westin|Manorhouse|Vermont|Allure|Ivy|Indulgence)-/i', $imgUrl) && stripos($imgUrl, 'Sono-Glass') === false && stripos($imgUrl, 'Sonno-Glass') === false && !preg_match('/Sono-Glass-Bed/i', $imgUrl)) continue;

                // Include if it's a real Sono Glass Bed image
                if (stripos($imgUrl, 'Sono') !== false || stripos($imgUrl, 'Sonno') !== false) {
        ...
        echo "Sono Glass Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
