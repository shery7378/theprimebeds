...
class AmbassadorDivanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Ambassador Divan Bed')->first();
        if (!$item) {
            echo "Ambassador Divan Bed not found. Trying like query.\n";
            $item = DB::table('items')->where('name', 'like', '%Ambassador Divan Bed%')
                      ->where('name', 'not like', '%Ottoman%')
                      ->first();
        }

        if (!$item) {
            echo "Ambassador Divan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ambassador_divan.json'));
        ...
        echo "Ambassador Divan Bed fully updated with attributes and images!\n";
    }
}
