...
class MountValeBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Mount Vale Bed%')->first();
        if (!$item) {
            echo "Mount Vale Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('mount-vale-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston|Westin|Manorhouse|Vermont|Allure|Ivy|Indulgence|Sono|Hyatt|Rosie|Merry-Kids|Dolsie|Fontaine)-/i', $imgUrl) && stripos($imgUrl, 'Mount-Vale') === false) continue;

                // Include if it's a real Mount Vale Bed image
                if (stripos($imgUrl, 'Mount-Vale') !== false) {
        ...
        echo "Mount Vale Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
