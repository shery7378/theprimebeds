...
class AvaGraceBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Ava Grace Bed')->first();
        if (!$item) {
            echo "Ava Grace Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ava-grace-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['60', '72', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'HEADBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Ava Grace Bed fully updated with attributes and images!\n";
    }
}
