...
class RichardsonRoyaleOttomanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Richardson Royale Ottoman Bed%')->first();
        if (!$item) {
            echo "Richardson Royale Ottoman Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('richardson_royale_ottoman.json'));
        ...
        $insertAttr = function($name, $options) use ($item) {
            ...
            foreach($options as $opt) {
                DB::table('attribute_options')->insert([
                    'attribute_id' => $attrId,
                    'name' => $opt['name'],
                    'price' => isset($opt['price']) ? (int)str_replace(['$', ','], '', $opt['price']) : 0,
                    'keyword' => Str::slug($opt['name']),
                    'stock' => 'unlimited',
                    'variation_images' => isset($opt['image']) && $opt['image'] ? json_encode([$opt['image']]) : '[]'
                ]);
            }
        };
        ...
        echo "Richardson Royale Ottoman Bed fully updated with attributes and images!\n";
    }
}
