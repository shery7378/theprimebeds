...
class MillhouseKidsBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Millhouse Kids Bed%')->first();
        if (!$item) {
            echo "Millhouse Kids Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('millhouse_kids.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['60', '50', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Millhouse Kids Bed fully updated with attributes and images!\n";
    }
}
