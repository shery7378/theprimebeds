...
class WestinBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Westin Bed%')->first();
        if (!$item) {
            echo "Westin Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('westin-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston-X|Hyatt)-/i', $imgUrl) && stripos($imgUrl, 'Westin') === false) continue;

                // Only include if it's a real Westin Bed image
                if (stripos($imgUrl, 'Westin') !== false) {
        ...
        echo "Westin Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
