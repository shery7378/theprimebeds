...
class MerryDeluxeBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Merry Deluxe Bed%')->first();
        if (!$item) {
            echo "Merry Deluxe Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('merry-deluxe-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie)-/i', $imgUrl) && stripos($imgUrl, 'Merry') === false) continue;

                // Only include if it's a real Merry Deluxe Bed image
                if (stripos($imgUrl, 'Merry') !== false) {
        ...
        echo "Merry Deluxe Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
