...
class ShakespeareAllFoamMattressSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Shakespeare All Foam Mattress (Firm)')->first();
        if (!$item) {
            echo "Shakespeare All Foam Mattress (Firm) not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('shakespeare-all-foam-mattress.json'));
        ...
        echo "Shakespeare All Foam Mattress fully updated with attributes and images!\n";
    }
}
