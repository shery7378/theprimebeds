...
class EchoBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Echo Bed%')->first();
        if (!$item) {
            echo "Echo Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('echo-bed.json'));
        ...
        // Determine main photo (skip luxlogo and dimension images)
        ...
        // Gallery – skip main photo to avoid duplicate display
        if (!empty($data['productImages'])) {
        ...
        echo "Echo Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
