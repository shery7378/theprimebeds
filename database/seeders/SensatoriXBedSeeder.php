...
class SensatoriXBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Sensatori-X Bed%')->first();
        if (!$item) {
            echo "Sensatori-X Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('sensatori_x.json'));
        ...
        echo "Sensatori-X Bed fully updated with attributes and images!\n";
    }
}
