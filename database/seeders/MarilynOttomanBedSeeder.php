...
class MarilynOttomanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Marilyn Ottoman Bed%')->first();
        if (!$item) {
            echo "Marilyn Ottoman Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('marilyn-ottoman-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Roselyn|Superior|Elegance|Ari|Ava|Lily|Sensatori|Ambassador|Amelia|Lottie|Court|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston|Westin|Manorhouse|Mount-Vale|Allure|Ivy|Indulgence|Sono|Hyatt|Rosie|Merry-Kids|Dolsie|Fontaine|Vermont|Perri|Charlie|Florence|Invicta|Bellagio|Valencia|Grantley)-/i', $imgUrl) && stripos($imgUrl, 'Marilyn') === false) continue;

                // Include if it's a real Marilyn Bed image
                if (stripos($imgUrl, 'Marilyn') !== false) {
        ...
        echo "Marilyn Ottoman Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
