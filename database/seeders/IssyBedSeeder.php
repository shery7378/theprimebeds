...
class IssyBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Issy Bed%')->where('name', 'not like', '%Kids%')->first();
        if (!$item) {
            echo "Issy Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('issy_bed.json'));
        ...
        echo "Issy Bed fully updated with attributes and images!\n";
    }
}
