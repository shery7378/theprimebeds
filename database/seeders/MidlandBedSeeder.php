...
class MidlandBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Midland Bed%')->first();
        if (!$item) {
            echo "Midland Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('midland-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan)-/i', $imgUrl) && stripos($imgUrl, 'Midland') === false) continue;

                // Only include if it's a real Midland Bed image
                if (stripos($imgUrl, 'Midland') !== false) {
        ...
        echo "Midland Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
