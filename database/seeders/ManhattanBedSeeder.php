...
class ManhattanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Manhattan Bed%')->first();
        if (!$item) {
            echo "Manhattan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('manhattan-bed.json'));
        ...
        // Gallery – only actual Manhattan Bed product photos
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/(IMPERIAL|Oxford|SHAKESPEARE|cloud-3000|AMBASSADOR|CELEBRATION|Bamboo)/i', $imgUrl)) continue;
                // Skip "no mattress" image
                if (stripos($imgUrl, '/no.png') !== false) continue;
                // Skip SVG icons
                ...
                // Skip other bed product images (not Manhattan)
                if (preg_match('/\/(Ambassador|Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior)-/i', $imgUrl) && stripos($imgUrl, 'Manhattan') === false) continue;
                // Skip shop icon
                if (stripos($imgUrl, 'shop-150x150') !== false) continue;
                // Only include if it's a real Manhattan Bed image
                if (stripos($imgUrl, 'Manhattan-Bed') !== false) {
        ...
        echo "Manhattan Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
