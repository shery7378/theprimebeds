...
class LylaLouiseBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Lyla Louise Bed%')->first();
        if (!$item) {
            echo "Lyla Louise Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('lyla-louise-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['36', '60', '72', '84', '28', '47', '54', '15', '16', '21', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'LENGTH (IN)', 'HEADBOARD HEIGHT (IN)', 'FOOTBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach($data['productImages'] as $imgUrl) {
                // SKIP duplicating the main photo in the gallery to avoid frontend double display
                if ($imgUrl !== $mainPhoto) {
        ...
        echo "Lyla Louise Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
