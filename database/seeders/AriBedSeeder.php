...
class AriBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Ari Bed%')->first();
        if (!$item) {
            echo "Ari Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ari-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['36', '60', '72', '84', '28', '54', '15', '16', '21', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'LENGTH (IN)', 'HEADBOARD HEIGHT (IN)', 'FOOTBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Ari Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
