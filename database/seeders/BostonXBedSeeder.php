...
class BostonXBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Boston-X Bed%')->first();
        if (!$item) {
            echo "Boston-X Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('boston-x-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (stripos($imgUrl, '/no.png') !== false || stripos($imgUrl, '/no-1.png') !== false || stripos($imgUrl, 'No-image-smaller') !== false) continue;
                // Skip SVG icons
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland)-/i', $imgUrl) && stripos($imgUrl, 'Boston-X') === false) continue;

                // Only include if it's a real Boston-X Bed image
                if (stripos($imgUrl, 'Boston-X') !== false) {
        ...
        echo "Boston-X Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
