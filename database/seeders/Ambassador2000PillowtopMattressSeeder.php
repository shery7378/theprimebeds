...
class Ambassador2000PillowtopMattressSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'Ambassador 2000 Pillowtop Mattress (Medium-Firm)')->first();
        if (!$item) {
            echo "Ambassador 2000 Pillowtop Mattress (Medium-Firm) not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ambassador-2000-pillowtop-mattress.json'));
        ...
        echo "Ambassador 2000 Pillowtop Mattress fully updated with attributes and images!\n";
    }
}
