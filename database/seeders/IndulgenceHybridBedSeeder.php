...
class IndulgenceHybridBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Indulgence Hybrid Bed%')->first();
        if (!$item) {
            echo "Indulgence Hybrid Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('indulgence-hybrid-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador)-/i', $imgUrl) && stripos($imgUrl, 'Indulgence') === false) continue;

                // Only include if it's a real Indulgence Hybrid Bed image
                if (stripos($imgUrl, 'Indulgence') !== false) {
        ...
        echo "Indulgence Hybrid Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
