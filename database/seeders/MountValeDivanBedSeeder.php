...
class MountValeDivanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Mount Vale Divan Bed%')->first();
        if (!$item) {
            echo "Mount Vale Divan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('mount_vale_divan.json'));
        ...
        $insertAttr('Base Type', $data['otherAddOns'] ?? []); // Divan base
        ...
        echo "Mount Vale Divan Bed fully updated with attributes and images!\n";
    }
}
