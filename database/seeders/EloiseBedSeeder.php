...
class EloiseBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Eloise Bed%')->first();
        if (!$item) {
            echo "Eloise Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('eloise-bed.json'));
        ...
        echo "Eloise Bed fully updated with attributes and images!\n";
    }
}
