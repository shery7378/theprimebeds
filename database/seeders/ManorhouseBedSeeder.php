...
class ManorhouseBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Manorhouse Bed%')->first();
        if (!$item) {
            echo "Manorhouse Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('manorhouse-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                // Exclude other beds explicitly (Ari, Ava, Avery, Crosby, Luciano, Manhattan, Marilyn, Rosie, Sono, Boston)
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe|Crosby|Manhattan|Midland|Boston|Westin|Manorhouse|Vermont|Allure|Ivy|Indulgence|Sono|Hyatt|Rosie)-/i', $imgUrl) && stripos($imgUrl, 'Manorhouse') === false) continue;

                // Include if it's a real Manorhouse Bed image
                if (stripos($imgUrl, 'Manorhouse') !== false) {
        ...
        echo "Manorhouse Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
