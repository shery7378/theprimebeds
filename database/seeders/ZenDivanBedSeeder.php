...
class ZenDivanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Zen Divan Bed%')->first();
        if (!$item) {
            echo "Zen Divan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('zen_divan_bed.json'));
        ...
        $insertAttr('Base Type', $data['otherAddOns'] ?? []); // Divan beds have Base Type
        ...
        echo "Zen Divan Bed fully updated with attributes and images!\n";
    }
}
