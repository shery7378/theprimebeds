...
class EmersonBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Emerson Bed%')->first();
        if (!$item) {
            echo "Emerson Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('emerson-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['36', '60', '72', '84', '54', '15', '16', '21', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'LENGTH (IN)', 'HEADBOARD HEIGHT (IN)', 'FOOTBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        if (!empty($data['productImages'])) {
            foreach($data['productImages'] as $imgUrl) {
                if (strpos($imgUrl, 'luxlogo') === false && strpos($imgUrl, 'Dimensions') === false) {
                    $mainPhoto = $imgUrl;
                    break;
                }
            }
        ...
        echo "Emerson Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
