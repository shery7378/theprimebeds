...
class DelilahBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Delilah Bed%')->first();
        if (!$item) {
            echo "Delilah Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('delilah-bed.json'));
        ...
        foreach($data['specs'] as $k => $v) {
            ...
            if (in_array($k, ['60', '72', '84', '54', '16', 'Product price:', 'Total options:', 'Order total:', 'Bed Size', 'Headboard Height (in)', 'BED SIZE', 'LENGTH (IN)', 'HEADBOARD HEIGHT (IN)', 'FOOTBOARD HEIGHT (IN)'])) continue;
            $specNames[] = $k;
            $specDescs[] = $v;
        }
        ...
        echo "Delilah Bed fully updated with attributes and images!\n";
    }
}
