...
class IvyDivanBedSeeder extends Seeder
{
    public function run(): void
    {
        $item = DB::table('items')->where('name', 'like', '%Ivy Divan Bed%')->first();
        if (!$item) {
            echo "Ivy Divan Bed not found.\n";
            return;
        }

        $jsonStr = file_get_contents(base_path('ivy-divan-bed.json'));
        ...
        if (!empty($data['productImages'])) {
            foreach ($data['productImages'] as $imgUrl) {
                if (stripos($imgUrl, 'luxlogo') === false && stripos($imgUrl, 'Dimensions') === false) {
                    $mainPhoto = $imgUrl;
                    break;
                }
            }
        ...
        if (!empty($data['productImages'])) {
            ...
            foreach ($data['productImages'] as $imgUrl) {
                ...
                if (preg_match('/(divan-no-drawer|divan-two-drawer|divan-left-drawer|divan-right-drawer)/i', $imgUrl)) continue;
                // Skip "no mattress" image
                if (stripos($imgUrl, '/no.png') !== false || stripos($imgUrl, '/no-1.png') !== false) continue;
                // Skip SVG icons
                ...
                if (preg_match('/\/(Avery|Delilah|Isabella|Lyon|Lyla|Luciano|Marilyn|Roselyn|Superior|Elegance|Ari|Ava|Lily|Mount-Vale|Sensatori|Ambassador|Amelia|Lottie|Court|Bellagio|Celine|Eloise|Richardson|Franklin|Tara-Luxe)-/i', $imgUrl) && stripos($imgUrl, 'Ivy-Divan') === false) continue;

                // Only include if it's a real Ivy Divan Bed image
                if (stripos($imgUrl, 'Ivy-Divan') !== false) {
        ...
        echo "Ivy Divan Bed fully updated with attributes, images, and correct main photo!\n";
    }
}
?>
