...
class ItemRepository
{
    ...
    public function store($request)
    {
        ...
        if ($file = $request->file('photo')) {
            $images_name = ImageHelper::ItemhandleUploadedImage($request->file('photo'), '');
        ...
        $input['variations'] = [
            'customization_level' => $request->input('customization_level'),
            'custom_text' => $request->input('custom_text'),
        ];
        $input['bundle_discount'] = [
            'discount_items' => $request->input('discount_items'),
            'discountItems_price' => $request->input('discountItems_price'),
        ];
        $curr = Currency::where('is_default', 1)->first();
        $input['discount_price'] = $request->discount_price / $curr->value;
        $input['previous_price'] = $request->previous_price / $curr->value;
        if ($request->has('purchase_price') && $request->purchase_price) {
            $input['purchase_price'] = $request->purchase_price / $curr->value;
        }
    ...
    public function update($item, $request)
    {
        ...
        if ($request->file('photo')) {
            ...
            $images_name = ImageHelper::ItemhandleUpdatedUploadedImage($request->photo, '', $item, '', 'photo');
            $input['photo'] = $images_name[0];
            $input['thumbnail'] = $images_name[1];
        }
    ...
    public function delete($item)
    {
        ...
        if ($item->item_type == 'digital' && $item->file) {
            ImageHelper::handleDeletedImage($item, 'file', '');
        }
        $item->delete();
    }
    ...
    public function storeImageData($request, $item_id = null)
    {
        ...
        if ($galleries = $request->file('galleries')) {
            foreach ($galleries as $key => $gallery) {
                $storeData[$key] = [
                    'photo' =>  ImageHelper::handleUploadedImage($gallery, ''),
