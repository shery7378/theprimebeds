...
trait PaypalCheckout
{
    ...
    public function __construct()
    {
        ...
        $this->gateway->setTestMode(
            $paydata["check_sandbox"] == 1 ? true : false,
        );
    }
    ...
    public function paypalSubmit($data)
    {
        ...
        foreach ($cart as $key => $item) {
            ...
            $realItemId = PriceHelper::GetItemId($key);
            $mainItem = ModelsItem::find($realItemId);
            if ($mainItem && $mainItem->tax) {
                $total_tax += $mainItem::taxCalculate($mainItem);
            }
        }
        $cart_total += $custom_total;
        ...
        $orderData["state_price"] = PriceHelper::StatePrce(
            $data["state_id"],
            $cart_total,
        );
    ...
    public function paypalNotify($responseData)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            if (!PriceHelper::Digital()) {
                ...
                $shipping = ShippingService::findOrFail(
                    $order_input_data["shipping_id"],
                );
            }
            $discount = [];
            if (Session::has("coupon")) {
            ...
            $orderData["txnid"] = $response->getData()["transactions"][0][
                "related_resources"
            ][0]["sale"]["id"];
            ...
            $new_txn = $new_txn =
                "ORD-" .
                str_pad(Carbon::now()->format("Ymd"), 4, "0000", STR_PAD_LEFT) .
                "-" .
                $order->id;
