...
trait MollieCheckout
{
    ...
    public function MollieSubmit($data)
    {
        ...
        foreach ($cart as $key => $item) {
            ...
            $realItemId = \App\Helpers\PriceHelper::GetItemId($key);
            $taxItem = Item::find($realItemId);
            if ($taxItem && $taxItem->tax) {
                $total_tax += $taxItem->tax->value;
            }
        }
    ...
    public function mollieNotify($responseData)
    {
        ...
        $grand_total = $grand_total - ($discount ? $discount["discount"] : 0);
        $total_amount = PriceHelper::setConvertPrice($grand_total);
        $orderData["state"] = $input_data["state_id"]
            ? json_encode(State::findOrFail($input_data["state_id"]), true)
            : null;
