...
class HomePageController extends Controller
{
    ...
    public function index(){
        ...
        return view('back.home-page.index',[
            'top_banner' => json_decode($data->top_banner,true),
            ...
            'home_menu_category' => json_decode($data->menu_category,true),
        ]);
    }
    ...
    public function top_banner_update(Request $request)
    {
        ...
        $data->update();
        return redirect()->back()->withSuccess(__('Top Banner Updated Successfully'));
    }
    ...
    public function first_banner_update(Request $request)
    {
        ...
        foreach($all_images_names as $single_image){
            if($request->hasFile($single_image)){
                ...
                $input[$single_image] = ImageHelper::handleUploadedImage($request->$single_image,'',$check[$single_image]);
            }else{
                $check = json_decode($data->banner_first,true);
    ...
    public function homepagemenuupdate(Request $request)
    {
       $category = json_encode($request->home_menu_category,true);
       $data = HomeCutomize::first();
       $data->menu_category = $category;
       $data->update();
       return redirect()->back()->withSuccess(__('Menu Update Successfully'));
