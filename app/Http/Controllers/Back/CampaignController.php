...
class CampaignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (CampaignItem::whereItemId($request->item_id)->exists()) {
            return redirect()
                ->route("back.campaign.index")
                ->withError(__("Allready Added This Product."));
        }
    ...
    public function destroy($id)
    {
        ...
        return redirect()
            ->route("back.campaign.index")
            ->withSuccess(__("Product Delete Successfully Successfully."));
    }
        ...
        if ($type == "is_feature" && $status == 1) {
            if (CampaignItem::whereIsFeature(1)->count() == 10) {
                return redirect()
                    ->route("back.campaign.index")
                    ->withError(
                        __("10 products are allready added to feature"),
                    );
            }
        }
